...
export const sellerAuthService = {
  register(payload: SellerRegisterPayload): Promise<ApiResponse<SellerRegisterResponseData>> {
    return apiClient<SellerRegisterResponseData>('/seller/auth/register', {
      method: 'POST',
      // Phase 38 — bake the seller-type discriminator into the body
      // so the backend can persist it on the new seller row. Header
      // is also sent by api-client but the body field is what gets
      // saved.
