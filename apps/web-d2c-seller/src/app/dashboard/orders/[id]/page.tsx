...
// Self-delivery UI hidden 2026-06-24 — D2C ships via Delhivery. The picker +
...
const { id } = useParams<{ id: string }>();
  ...
  const handleRequestPickup = async () => {
    ...
    try {
      const res = await apiClient<{ message?: string }>(
        `/seller/sub-orders/${id}/request-pickup`,
        {
          method: 'POST',
          headers: { 'X-Idempotency-Key': `pickup-${id}-${Date.now()}` },
        },
      );
      void notify(res?.message || 'Pickup requested with Delhivery.');
      fetchOrder();
    } catch (err) {
      void notify(
  ...
  // Self-service shipping label — the seller packs the box, so they need to
  // print the Delhivery label and paste it on before pickup. Fetches the real
  // carrier label URL on demand and opens the PDF in a new tab. The label can
  // legitimately be empty right after booking (carrier propagation), so we show
  // a friendly "try again" message instead of opening an undefined URL.
  const handleDownloadLabel = async () => {
    setActionLoading('label');
    try {
      const res = await apiClient<{ labelUrl?: string | null }>(
        `/seller/sub-orders/${id}/label`,
      );
      const url = res?.data?.labelUrl;
      if (!url) {
        void notify(
          'The courier AWB is still being booked (this takes a few seconds after “Mark as Packed”). Refresh the page in a moment and the label will be ready.',
        );
        return;
      }
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20, paddingTop: 16, borderTop: '1px solid #f3f4f6' }}>
              ...
              {/* Fulfillment status progression - PACK. Delhivery auto-ships
                  ...
                  required BEFORE packing (the backend enforces this too). The
                  button is disabled with a count until the photos are uploaded
                  via the Shipment Evidence panel below. */}
              {canMarkPacked && (
                <button
              ...
              {/* Self-service Delhivery pickup — the seller schedules their own
                  pickup so they don't have to ask an admin (and risk it being
                  forgotten). Shown for Delhivery orders once packed/shipped. */}
              {isDelhivery &&
                (order.fulfillmentStatus === 'PACKED' ||
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      fontWeight: 600,
                      background: actionLoading === 'pickup' ? '#93c5fd' : '#2563eb',
                      color: '#fff',
                      border: 'none',
                      borderRadius: 8,
                      cursor: actionLoading === 'pickup' ? 'not-allowed' : 'pointer',
                      alignSelf: 'flex-start',
                    }}
                  >
                    {actionLoading === 'pickup'
                      ? 'Requesting pickup…'
                      : '\u{1F69A} Request Delhivery pickup'}
                  </button>
                )}

              {/* Download shipping label — secondary action next to pickup. The
                  seller prints this and pastes it on the box before pickup.
                  Gated on trackingNumber: the label is built from the booked
                  AWB, which the auto-book handler attaches a few seconds AFTER
                  "Mark as Packed". Showing the button before the AWB lands led
                  to a dead-click + a misleading "not generated yet" toast, so we
                  only surface it once the AWB exists (the info box above covers
                  the in-between "courier is being booked" moment). */}
              {isDelhivery &&
                ...
                (order.fulfillmentStatus === 'PACKED' ||
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      cursor: actionLoading === 'label' ? 'not-allowed' : 'pointer',
                      alignSelf: 'flex-start',
                    }}
                  >
                    {actionLoading === 'label'
                      ? 'Fetching label…'
                      : '\u{1F3F7}\u{FE0F} Download shipping label'}
                  </button>
                )}
 ...
 * currently commented out — D2C ships via Delhivery). Hidden for rejected /
...
function DeliverySection({
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {/* Self-delivery picker + status buttons — hidden 2026-06-24 (D2C ships via
          Delhivery). Uncomment this block, the imports, and the derived flags
          above to restore.
