  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 880 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: 20, marginBottom: 16 }}>
        ...
        <p style={{ marginTop: 4, fontSize: 12, color: '#525A65' }}>Opened {new Date(ticket.createdAt).toLocaleString('en-IN')}</p>
      </div>
...
function Bubble({ message, myName }: { message: TicketMessage; myName: string }) {
  const isMine = message.senderType === 'FRANCHISE';
  return (
    <div style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start' }}>
      <div style={{ maxWidth: '78%' }}>
        <div style={{ fontSize: 12, color: '#525A65', marginBottom: 4, textAlign: isMine ? 'right' : 'left' }}>
          <strong style={{ color: '#0F1115' }}>{isMine ? myName : message.senderName}</strong>
          {' · '}
