...
export default function NewFranchiseTicketPage() {
  ...
  useEffect(() => {
    franchiseSupportService.listCategories().then((res) => res.data && setCategories(res.data)).catch(() => {});
  }, []);

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    // Field-level length checks so a near-empty subject/body can't be
    // submitted. The textarea caps length at 5000; enforce a sensible
    // minimum so agents get enough context to act on the ticket.
    const subjectError = validateText(subject, {
      min: 5,
      max: 200,
      label: 'Subject',
    });
    if (subjectError) return setError(subjectError);
    const bodyError = validateText(body, {
      min: 10,
      max: 5000,
      label: 'Description',
    });
    if (bodyError) return setError(bodyError);
    setSubmitting(true);
    try {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 720 }}>
      ...
      <p style={{ marginTop: 4, fontSize: 14, color: '#525A65' }}>We typically reply within one business day.</p>
      ...
      <form onSubmit={submit} style={{ marginTop: 24, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: 20 }}>
        <Field label="Subject">
          <input type="text" maxLength={200} value={subject} onChange={(e) => setSubject(e.target.value)} disabled={submitting}
            placeholder="e.g. Procurement order delivery issue" style={input} />
        </Field>
        ...
        <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <Link href="/dashboard/support" style={{ height: 40, padding: '0 16px', display: 'inline-flex', alignItems: 'center', border: '1px solid #D2D6DC', background: '#fff', color: '#0F1115', borderRadius: 9999, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Cancel</Link>
