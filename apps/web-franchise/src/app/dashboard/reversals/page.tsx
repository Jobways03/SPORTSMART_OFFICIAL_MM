...
import {
  franchiseReversalsService,
  FranchiseReversal,
} from '@/services/reversals.service';
...
export default function FranchiseReversalsPage() {
  const [rows, setRows] = useState<FranchiseReversal[]>([]);
  ...
  const fetchRows = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await franchiseReversalsService.list({ limit: 100 });
  ...
  const cancel = async (id: string) => {
    ...
    setError(null);
    try {
      await franchiseReversalsService.cancel(id);
      await fetchRows();
    } catch (err) {
      setError(
        err instanceof ApiError
