...
// Self-delivery UI removed from the franchise portal (2026-06-03) — franchise
// ships via Delhivery only (matches the seller/retail Delhivery flow). The
// DeliveryMethodPicker / SelfDeliveryStatusButtons components are no longer used.
...
const { id } = useParams<{ id: string }>();
  ...
  const handleRequestPickup = async () => {
    ...
    try {
      const res = await franchiseOrdersService.requestPickup(id);
      void notify(res?.message || 'Pickup requested with Delhivery.');
      fetchOrder();
    } catch (err) {
      if (err instanceof ApiError) void notify(err.body.message || 'Failed to request pickup');
      else void notify('Failed to request pickup');
    } finally {
      setActionLoading(null);
    }
  };

  // Self-service shipping label — the franchise packs the box, so it needs to
  // print the Delhivery label and paste it on before pickup. Opens the real
  // carrier PDF; labelUrl can be null right after booking, so notify instead of
  // opening an undefined URL.
  const handleDownloadLabel = async () => {
    if (!id) return;
    setActionLoading('label');
    try {
      const res = await franchiseOrdersService.getShippingLabel(id);
      const url = res?.data?.labelUrl;
      if (!url) {
        void notify(
          'Label not generated by Delhivery yet — available once the shipment is manifested / picked up. Try again in a moment.',
        );
        return;
      }
      window.open(String(url), '_blank', 'noopener,noreferrer');
    } catch (err) {
      if (err instanceof ApiError) void notify(err.body.message || 'Failed to fetch the label');
      else void notify('Failed to fetch the label');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  const canMarkShipped =
    ...
    // Delhivery auto-ships on PACK (the AWB is auto-assigned), so the manual
    // "Mark as Shipped" tracking-entry flow is hidden for Delhivery orders —
    // the franchise just packs and (optionally) requests the pickup.
      ...
      >
        ...
        <div style={{ flex: '0 0 320px', minWidth: 280, position: 'sticky', top: 80 }}>
          <div className="card">
            ...
            {/* Delhivery self-service pickup — the franchise schedules their own
                pickup instead of asking an admin (and risking it being forgotten). */}
            {order.deliveryMethod === 'DELHIVERY' &&
              (order.fulfillmentStatus === 'PACKED' ||
            ...
            {/* Download shipping label — the franchise prints this and pastes
                it on the box before pickup. */}
            {order.deliveryMethod === 'DELHIVERY' &&
              (order.fulfillmentStatus === 'PACKED' ||
                order.fulfillmentStatus === 'SHIPPED') && (
                <button
...
function FranchiseDeliverySection({
  ...
  // Franchise orders ship via Delhivery (the platform courier) only — there is
  // no self-delivery option for franchise (matches the seller/retail Delhivery
  // flow). Auto-ship books the AWB on PACK; the "Request pickup" button in the
  // Actions card lets the franchise schedule the Delhivery pickup themselves.
