...
export const franchiseSupportService = {
  listCategories(): Promise<ApiResponse<TicketCategory[]>> {
    return apiClient<TicketCategory[]>('/franchise/support/categories');
  },
  listMyTickets(
    page = 1,
    limit = 20,
    ...
    return apiClient<TicketListPage>(`/franchise/support/tickets?${qs.toString()}`);
  },
  getTicket(id: string): Promise<ApiResponse<TicketDetail>> {
    return apiClient<TicketDetail>(`/franchise/support/tickets/${id}`);
  },
  createTicket(payload: CreateTicketPayload): Promise<ApiResponse<Ticket>> {
    return apiClient<Ticket>('/franchise/support/tickets', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  },
  reply(ticketId: string, body: string): Promise<ApiResponse<TicketDetail>> {
    return apiClient<TicketDetail>(`/franchise/support/tickets/${ticketId}/messages`, {
      method: 'POST',
      body: JSON.stringify({ body }),
    });
  },
  closeTicket(ticketId: string): Promise<ApiResponse<Ticket>> {
    return apiClient<Ticket>(`/franchise/support/tickets/${ticketId}/close`, {
      method: 'POST',
    });
  },
};
