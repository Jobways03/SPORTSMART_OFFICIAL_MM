...
export default function EditBankModal({ franchise, initial, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await adminFranchisesService.updateBankDetails(franchise.id, {
  ...
  const initials = franchise.ownerName
    .split(' ')
    .map((w) => w[0])
    .slice(0, 2)
    .join('')
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="modal-warning">
            This changes where the franchise&apos;s payouts are settled. Re-enter the full account
            number (only the last 4 digits are stored in plain text).
          </div>
