...
export default function ApproveReturnModal({
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await franchiseReturnsService.approve(returnId, franchiseId, notes || undefined);
      onSuccess();
    } catch (err) {
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 16 }}>
            ...
            <strong style={{ color: 'var(--color-text)' }}>{returnNumber}</strong>. Once
            approved the customer can initiate pickup.
          </div>
