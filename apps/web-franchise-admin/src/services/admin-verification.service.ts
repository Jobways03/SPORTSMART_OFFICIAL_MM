...
export interface QueueStats {
  ...
  unclaimedRed: number;
  // Phase 174 — CRITICAL is the new top band; UNSCORED counts orders the
  // scorer hasn't reached yet. Both are already returned by queue-stats.
...
// Phase 174 — CRITICAL sits above RED as the highest severity band.
export type RiskBand = 'GREEN' | 'YELLOW' | 'RED' | 'CRITICAL';

/**
 * Band filter accepted by GET /admin/verification/orders. In addition to the
 * four concrete bands, HIGH = RED+CRITICAL, RED_YELLOW = YELLOW+RED+CRITICAL,
 * UNSCORED = not-yet-scored, ALL = no filter.
 */
...
/** Bands that can be passed to claim-next ("claim next RED"). */
export type ClaimableBand = 'GREEN' | 'YELLOW' | 'RED' | 'CRITICAL';

/** One row of GET /admin/verification/orders. Money stays a decimal string. */
...
export interface ListVerificationOrdersParams {
 ...
 * Pass a `band` to claim the next order of that band first ("claim next RED");
 * with no band the backend claims the oldest unclaimed regardless of risk.
 */
...
/**
 * GET /admin/verification/orders — band-filtered, paginated list of PLACED
 * orders awaiting verification (highest risk first). Read-only.
 */
export function listVerificationOrders(
  params: ListVerificationOrdersParams = {},
 ...
 * acting. An optional `reason` (3..500 chars) is audited alongside the
 * old→new band. Returns the new RiskInfo.
 */
