...
export default function AdminVariantDetailPage() {
  ...
  const currentVariant = product?.variants?.find((v: any) => v.id === variantId) || null;
  ...
  const loadProduct = useCallback(async () => {
    try {
      ...
      if (res.data) {
        setProduct(res.data);
        const variant = (res.data as any).variants?.find((v: any) => v.id === variantId);
  ...
  }, [productId, variantId, populateForm, router]);
  ...
  function getVariantImage(v: any): string | null {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await adminProductsService.updateVariant(productId, variantId, payload);
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of validFiles) {
      try {
        await adminProductsService.uploadVariantImage(productId, variantId, file);
        uploaded++;
      } catch {
        failed++;
      }
  ...
  async function handleMoveImage(imageId: string, direction: 'up' | 'down') {
    ...
    try {
      await adminProductsService.reorderVariantImages(productId, variantId, newOrder);
      await loadProduct();
    } catch {
  ...
  if (loadError) {
    return (
      <div className="product-form-page">
        ...
        <div className="form-card"><p style={{ color: 'var(--color-error, #dc2626)', fontSize: 14 }}>{loadError}</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="product-form-page">
      ...
      <div className="variant-detail-layout">
        ...
        <div className="variant-detail-right">
          <div className="form-card">
            ...
            {currentVariant.masterSku && (
              <div className="form-group" style={{ marginBottom: 12 }}>
                <label className="form-label">Master SKU</label>
                <input
                  type="text"
                  className="form-input"
                  value={currentVariant.masterSku}
                  readOnly
                  style={{ background: '#f0fdf4', color: '#15803d', fontFamily: 'monospace', fontWeight: 600 }}
                />
              </div>
            )}
          ...
          <div className="form-card">
            ...
            {/*
              ...
              Compare at Price. The old "Platform Price
              (Customer-Facing)" field is gone — customers now see the
              seller's price directly.
            */}
            ...
            <div className="form-group">
              ...
              <div className="input-with-prefix">
                <span className="input-prefix">&#8377;</span>
                <input type="number" className="form-input" value={form.compareAtPrice} onChange={e => setForm(prev => ({ ...prev, compareAtPrice: e.target.value }))} placeholder="0.00" min="0" step="0.01" />
              </div>
              <span className="form-hint">Shown as strikethrough next to the seller&apos;s price.</span>
            </div>
            <div className="form-group">
              ...
              <span className="form-hint">
                Display-only informational cost. Not used by any pricing logic, including
                procurement. Use Procurement Price below for the procurement default.
              </span>
            </div>
            <div className="form-group">
              ...
              <span className="form-hint">
                Platform-wide default landed cost applied to all franchises. Pre-fills
                the approval modal for every franchise&apos;s procurement of this SKU, and is
                overwritten automatically when an approval saves a different value.
                Per-franchise overrides (set on each franchise&apos;s Procurement Pricing
                page) take precedence over this.
              </span>
            </div>
          </div>
          ...
          <div className="form-card">
            ...
            <div className="form-group">
              ...
              <input type="text" className="form-input" value={form.sku} onChange={e => setForm(prev => ({ ...prev, sku: e.target.value }))} placeholder="Stock keeping unit" />
            </div>
            <div className="form-group">
              <label className="form-label">Barcode</label>
              <input type="text" className="form-input" value={form.barcode} onChange={e => setForm(prev => ({ ...prev, barcode: e.target.value }))} placeholder="UPC, EAN, ISBN, etc." />
            </div>
            <div className="form-group">
              <label className="form-label">Quantity <span className="required">*</span></label>
              <input type="number" className="form-input" value={form.stock} onChange={e => setForm(prev => ({ ...prev, stock: e.target.value }))} placeholder="0" min="0" step="1" />
            </div>
          </div>
          ...
          <div className="form-card">
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={form.weightUnit} onChange={e => setForm(prev => ({ ...prev, weightUnit: e.target.value }))}>
            ...
            <div className="form-group">
              ...
              <input type="number" className="form-input" value={form.weight} onChange={e => setForm(prev => ({ ...prev, weight: e.target.value }))} placeholder="0" min="0" step="0.01" />
            </div>
            <div className="form-group">
              <label className="form-label">Dimensions (L x W x H)</label>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input type="number" className="form-input" style={{ flex: 1 }} value={form.length} onChange={e => setForm(prev => ({ ...prev, length: e.target.value }))} placeholder="L" min="0" step="0.1" />
                <span style={{ color: '#9ca3af' }}>&times;</span>
                <input type="number" className="form-input" style={{ flex: 1 }} value={form.width} onChange={e => setForm(prev => ({ ...prev, width: e.target.value }))} placeholder="W" min="0" step="0.1" />
                <span style={{ color: '#9ca3af' }}>&times;</span>
                <input type="number" className="form-input" style={{ flex: 1 }} value={form.height} onChange={e => setForm(prev => ({ ...prev, height: e.target.value }))} placeholder="H" min="0" step="0.1" />
                <select className="form-select" value={form.dimensionUnit} onChange={e => setForm(prev => ({ ...prev, dimensionUnit: e.target.value }))} style={{ width: 70 }}>
      ...
      <div className="form-actions">
        <button type="button" className="form-btn" onClick={() => router.push(`/dashboard/products/${productId}/edit`)}>
          Cancel
        </button>
        <button type="button" className="form-btn primary" onClick={handleSave} disabled={saving}>
