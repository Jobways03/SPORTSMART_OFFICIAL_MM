...
export class EInvoiceRetryCron {
  ...
  enabled(): boolean {
    return this.env.getBoolean('TAX_EINVOICE_RETRY_CRON_ENABLED' as any, true);
  }

  private retryCap(): number {
    return this.env.getNumber('TAX_EINVOICE_RETRY_CAP' as any, 5);
  }

  private cooldownMinutes(): number {
    return this.env.getNumber('TAX_EINVOICE_RETRY_COOLDOWN_MINUTES' as any, 5);
  }

  private scanLimit(): number {
    return this.env.getNumber('TAX_EINVOICE_RETRY_SCAN_LIMIT' as any, 50);
  }
