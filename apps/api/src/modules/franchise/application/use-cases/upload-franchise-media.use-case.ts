...
export class UploadFranchiseMediaUseCase {
  ...
  async execute(
    ...
    try {
      ...
      this.logger.error(`media upload failed for franchise ${franchiseId}: ${error?.message}`);
      throw new AppException(
    ...
    try {
      ...
      this.logger.error(
        `DB update failed after media upload for franchise ${franchiseId}: ${error?.message}`,
      );
