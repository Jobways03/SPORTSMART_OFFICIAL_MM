...
function ProductsContent({ initialData }: { initialData?: InitialData }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  // Phase 192 (#1) — when the server pre-fetched this page, skip the first
  // client fetch so the SSR-rendered grid (crawlable) is reused as-is.
  ...
  // of being folded into the free-text search.
  ...
  const [products, setProducts] = useState<ProductCardData[]>(initialData?.products ?? []);
  ...
  useEffect(() => {
    // #1 — the server already rendered this exact view; don't refetch it.
...
// Phase 192 (#1) — exported as a named client component; the server
// `page.tsx` wraps it (with metadata + JSON-LD) and seeds `initialData`.
export function ProductsClient({ initialData }: { initialData?: InitialData }) {
  return (
    <Suspense
      fallback={
        <StorefrontShell>
