...
/**
 * Renders one row per partner whose facade catalogue entry advertises
 * `capabilities.warehouseRegistration === 'REQUIRED'`. Each row shows
 * the current status from `GET /admin/logistics-partner/sellers/:id/
 * registrations` and offers an action button that POSTs to
 * `/admin/logistics-partner/sellers/:id/partners/:code/register`.
 *
 * The component fetches both sources on mount, joins them client-side,
 * and re-fetches the registrations list after every action. New
 * partners that appear in the facade catalogue automatically show up
 * here — no frontend change needed.
 */
export function PartnerRegistrationPanel({
    ...
    try {
      const [partnersRes, regsRes] = await Promise.all([
        ...
        apiClient<SellerRegistrationItem[]>(
          `/admin/logistics-partner/sellers/${encodeURIComponent(sellerId)}/registrations`,
        ),
      ]);
  ...
  const rows: RowState[] = useMemo(() => {
    ...
    return partners
      .filter(
        (p) => p.capabilities.warehouseRegistration === 'REQUIRED',
      )
      .map((p) => {
        const reg =
          registrations?.find((r) => r.partner === p.code) ?? null;
    ...
    async (code: string) => {
      ...
      try {
        const res = await apiClient<RegisterResponse>(
          `/admin/logistics-partner/sellers/${encodeURIComponent(sellerId)}/partners/${encodeURIComponent(code)}/register`,
  ...
  if (loading) {
    return (
      <div style={styles.loading}>Loading partner registrations…</div>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <div style={styles.empty}>
        No logistics partners require pickup-location registration right
        now.
      </div>
    );
  }
