...
//
// Buyer-side B2B GSTIN verification. Same flow as seller-gstins:
...
export default function CustomerTaxProfilesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setMsg(null);
    try {
      const res = await adminTaxService.listCustomerTaxProfiles();
  ...
  const counts = useMemo(() => {
    const verified = items.filter((p) => p.isVerified).length;
    const unverified = items.length - verified;
    const mismatch = items.filter((p) => lastOutcomes[p.id]?.legalNameMismatch).length;
    const defaults = items.filter((p) => p.isDefault).length;
    return { verified, unverified, mismatch, defaults };
  }, [items, lastOutcomes]);

  const filtered = useMemo(() => {
    let out = items;
    if (tab === 'VERIFIED')   out = out.filter((p) => p.isVerified);
    if (tab === 'UNVERIFIED') out = out.filter((p) => !p.isVerified);
    if (tab === 'MISMATCH')   out = out.filter((p) => lastOutcomes[p.id]?.legalNameMismatch);
    if (search.trim()) {
    ...
    // Sort: unverified first, then newest first.
    out = [...out].sort((a, b) => {
    ...
    <div style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: '#0F1115' }}>
          Customer tax-profile verification
        </h1>
        <p style={{ marginTop: 6, fontSize: 13, color: '#525A65', maxWidth: 760, lineHeight: 1.5 }}>
          Verify B2B customer GSTINs against the GSTN portal before their orders attract Tax
          Invoices. Unverified profiles still accept orders — this is where finance attests them
          for STRICT-mode invoicing.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        <div style={{ position: 'relative', flex: '1 1 280px', maxWidth: 460 }}>
          <input
            ...
            placeholder="Search GSTIN, legal name, customer, email, state…"
...
function KpiStrip({
  ...
  counts: { verified: number; unverified: number; mismatch: number; defaults: number };
  loading: boolean;
}) {
  return (
    <div style={{
      ...
      <Kpi label="Unverified"
        ...
        loading={loading}
        hint="Won't get a Tax Invoice under STRICT mode." />
      <Kpi label="Verified"
      ...
      <Kpi label="Default profiles"
        value={counts.defaults.toLocaleString('en-IN')}
        tone="neutral" loading={loading}
        hint="Auto-attached to new orders when present." />
    </div>
  );
}
    ...
    <tr style={{ borderTop: '1px solid #F3F4F6' }}>
      <td style={td}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#0F1115' }}>
          {name || <span style={{ color: '#7A828F' }}>—</span>}
        </div>
        {item.customer?.email && (
          <div style={{ fontSize: 12, color: '#525A65', marginTop: 2 }}>
            {item.customer.email}
          </div>
        )}
      ...
      <td style={{ ...td, whiteSpace: 'nowrap' }}>
        <button
          ...
          disabled={busy}
          style={busy
            ? { ...(item.isVerified ? btnSecondary : btnPrimary), ...busyStyle }
            : (item.isVerified ? btnSecondary : btnPrimary)}
        >
          {busy ? (
            <>Verifying…</>
          ) : item.isVerified ? (
...
function VerificationCell({
  item, mismatch,
}: { item: CustomerTaxProfileItem; mismatch: boolean }) {
  if (!item.isVerified) {
    return (
      <span style={{
...
function EmptyState({ tab, hasSearch }: { tab: Tab; hasSearch: boolean }) {
  let text: string;
  if (hasSearch) text = 'No customer tax profiles match your search.';
  else if (tab === 'UNVERIFIED') text = 'Every B2B customer profile is verified. Nothing to chase.';
  else if (tab === 'VERIFIED') text = 'No verified customer profiles yet.';
  else if (tab === 'MISMATCH') text = 'No legal-name mismatches recorded in this session.';
  else text = 'No B2B customer profiles on file yet.';
