// Phase B (P0.1, P0.5) — Discount & GST Breakdown card.
//
// Renders the per-order, per-item, per-tax, and per-liability ledger
// rows on the admin order-detail page. Source of truth is the
// allocation pipeline that runs at checkout — values here are
// historical snapshots (survive later edits to the parent Discount).
//
// Empty for legacy orders without allocation rows; the parent page
// only mounts this card when at least one row exists.
...
const fmtPaise = (v: string | number): string => {
  // BigInt values are serialized to strings on the wire (Prisma's
  // BigInt → JSON convention). Display rupees-with-paise (₹X.XX).
  const n = Number(v);
...
export function DiscountGstBreakdownCard({ breakdown, orderItems }: Props) {
  ...
  // Roll up liability ledger by party for the summary line. We use
  // plain `number` arithmetic here (paise as integers) — order-level
  // sums won't realistically approach 2^53 paise (~₹90 lakh crore).
