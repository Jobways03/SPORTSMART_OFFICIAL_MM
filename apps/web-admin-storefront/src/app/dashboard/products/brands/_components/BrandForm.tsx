...
export default function BrandForm({ brandId }: { brandId?: string }) {
  ...
  useEffect(() => {
    if (!brandId) return;
    apiClient<{ brand: BrandDetail }>(`/admin/brands/${brandId}`)
      .then((res) => {
        if (res.data?.brand) {
    ...
    apiClient<{ products: any[] }>(`/admin/products?${params}`)
      .then((res) => {
        ...
        const mapped = (res.data?.products || []).map((p: any) => ({
          id: p.id, title: p.title, imageUrl: p.primaryImageUrl || p.images?.[0]?.url || null,
        }));
  ...
  const handleBrowseAdd = async () => {
    const newIds = [...browseSelected].filter((id) => !products.some((p) => p.id === id));
    if (newIds.length > 0 && isEdit) {
      await apiClient(`/admin/brands/${brandId}/products`, {
        method: 'POST', body: JSON.stringify({ productIds: newIds }),
      });
      const res = await apiClient<{ brand: BrandDetail }>(`/admin/brands/${brandId}`);
      if (res.data?.brand) setProducts(res.data.brand.products || []);
    } else if (newIds.length > 0) {
      const newProducts = browseProducts.filter((bp) => newIds.includes(bp.id)).map((bp) => ({
        id: bp.id, title: bp.title, slug: '', status: 'ACTIVE', basePrice: null, images: bp.imageUrl ? [{ url: bp.imageUrl }] : [],
      }));
  ...
  const handleRemoveProduct = async (productId: string) => {
    if (isEdit) await apiClient(`/admin/brands/${brandId}/products/${productId}`, { method: 'DELETE' });
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: 80, color: '#9ca3af', fontSize: 14 }}>Loading brand...</div>;
  ...
  return (
    <div style={{ maxWidth: 1060, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
        <Link href="/dashboard/products/brands" style={{ color: '#6b7280', textDecoration: 'none', fontSize: 14 }}>
          &#8592;
        </Link>
          ...
          <section style={card}>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '36px 20px' }}>
                ...
                <div style={{ fontSize: 14, color: '#6b7280', fontWeight: 500 }}>There are no products in this brand.</div>
              ...
              <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
                {products.map((p, i) => (
                  ...
                  >
                    ...
                    <button onClick={() => handleRemoveProduct(p.id)} style={removeBtn} title="Remove">&times;</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {(name || pageTitle) ? (
              <div style={{ marginTop: 14 }}>
                ...
                <div style={{ fontSize: 12, color: '#006621', marginTop: 2 }}>
                  https://sportsmart.com &rsaquo; brands &rsaquo; {computedSlug}
                </div>
                {metaDescription && (
              ...
              <p style={{ fontSize: 13, color: '#9ca3af', margin: '12px 0 0' }}>
                Add a title and description to see how this brand might appear in a search engine listing
              </p>
            )}
              ...
              <div style={{ borderTop: '1px solid #e5e7eb', marginTop: 18, paddingTop: 18 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex' }}>
                    <span style={slugPrefix}>brands/</span>
                    <input
                      type="text"
                      value={urlHandle}
                  ...
                  <span style={charCount}>https://sportsmart.com/brands/{computedSlug}</span>
                </div>
              </div>
            )}
          </section>
