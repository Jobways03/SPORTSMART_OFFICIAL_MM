...
export const adminSellersService = {
  listSellers(params: ListSellersParams = {}): Promise<ApiResponse<SellerListResponse>> {
    ...
    // Phase 38 — D2C admin only ever lists D2C sellers. The header
    // already carries this scope; the explicit query param makes the
    // filter visible in server logs + uses the same code path as
    // any other admin filter.
  ...
  // verifyGstin marks the GSTIN verified for invoicing. Both gated on
  // sellers.approve, idempotent.
