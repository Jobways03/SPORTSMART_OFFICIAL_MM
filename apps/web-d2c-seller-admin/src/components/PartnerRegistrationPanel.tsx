...
/**
 * D2C seller-admin variant of the partner-registration panel. The staff
 * member is logged in as a SportsMart admin and operates on a specific
 * brand's behalf, so the panel takes `sellerId` as a prop (resolved by
 * the mounting page from a `?sellerId=` query param) and calls the
 * admin endpoints. The `X-Seller-Type: D2C` header is attached
 * automatically by the app's apiClient.
 *
 * Endpoints:
...
export function PartnerRegistrationPanel({
  ...
  const onUpdateAddress = useCallback(
    async (code: string) => {
      ...
      try {
        const res = await apiClient<RegisterResponse>(
          `/admin/logistics-partner/sellers/${encodeURIComponent(sellerId)}/partners/${encodeURIComponent(code)}/update-address`,
          { method: 'POST', body: JSON.stringify({}) },
        );
        if (res.data && res.data.ok === false) {
          setRowError((prev) => ({
...
// This is an infra condition the admin can't fix by editing the seller, so we
// don't show a scary red error — we explain it and offer Retry + an ops hint.
