...
export default function EditProductPage() {
  ...
  return (
    <div className="product-form-page">
      ...
      {/* Seller info (readonly) — product owner identity strip */}
      {product.seller && (
        <div className="seller-identity">
          <div className="seller-identity-avatar" aria-hidden="true">
            {(product.seller.sellerShopName || product.seller.sellerName || '?')
              .charAt(0)
              .toUpperCase()}
          </div>
          <div className="seller-identity-body">
            <div className="seller-identity-name">
              {product.seller.sellerName}
              {product.seller.sellerShopName && (
                <span className="seller-identity-shop"> &middot; {product.seller.sellerShopName}</span>
              )}
            </div>
            <div className="seller-identity-email">{product.seller.email}</div>
          </div>
          <span className="seller-identity-tag">Product owner</span>
        </div>
      )}
