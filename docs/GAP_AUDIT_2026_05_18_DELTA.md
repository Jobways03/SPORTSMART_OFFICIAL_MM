...
17. **Tax UI sweep — 10 items closed** in one session:
    ...
    - **HIGH #3 Seller-admin order tax** — `DiscountGstBreakdownCard` ported into web-d2c-seller-admin, mounted on order detail page.
...
| MED | A11Y | Modal overlays have onClick but no keyboard handlers (Escape close, focus trap) | `web-admin-storefront/.../products/components/delete-modal.tsx:36-37`, similar pattern across web-d2c-seller-admin | Keyboard-only users can't dismiss modals. |
